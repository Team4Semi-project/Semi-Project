...
public class Pagination {
	...
	private void calculate() {
		...
		// prevPage : 이전 페이지 번호 목록 중 끝 번호
		if(currentPage <= pageSize) {prevPage = 1;} // 이전으로 갈 페이지가 없을 경우
		else {prevPage = startPage - 1;}
		
		// nextPage : 다음 페이지 번호 목록 중 시작 번호
		if(endPage == maxPage) {nextPage = maxPage;} // 다음으로 넘어갈 페이지가 없을 경우
